...
class BenchmarkTab(BaseTab):
    ...
    def _run_benchmark(
        ...
        try:
            self.state.processes.benchmark = subprocess.Popen(
                cmd,
            ...
            if self.state.processes.benchmark.stdout:
                for line in iter(self.state.processes.benchmark.stdout.readline, ""):
                    if line:
                        yield line
                    if self.state.processes.benchmark.poll() is not None:
                        break

            remaining, _ = self.state.processes.benchmark.communicate()
            if remaining:
                yield remaining

            exit_code = self.state.processes.benchmark.returncode
            if exit_code == 0:
            ...
            self.state.processes.benchmark = None
