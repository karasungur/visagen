...
Key Features:
    - Morph factor controls the blend between src and dst identity
    - During training: random binomial sampling for morph
    - During inference: continuous morph_value for fine control
...
def morph_code(
    ...
    code_dst: torch.Tensor,
    morph_factor: float = 0.5,
    morph_value: float | None = None,
    training: bool = True,
) -> torch.Tensor:
    """
    Morph between source and destination codes.

    Args:
        code_src: Source identity code from inter_src.
        code_dst: Destination identity code from inter_dst.
        morph_factor: Probability for binomial sampling during training.
        morph_value: Fixed morph value for inference (0=dst, 1=src).
...
class AMPArchi(nn.Module):
    """
    AMP (Amplified Morphable Portrait) Architecture.

    AMP allows controllable morphing between source and destination faces.
    Uses separate inter modules for src and dst with a shared decoder.

    Architecture:
    - Shared Encoder
    - InterSrc - maps encoder output to source identity
    - InterDst - maps encoder output to destination identity
    - Shared Decoder - decodes morphed code
    ...
    def InterSrc(self) -> nn.Module:
        """Create source inter module."""
        return _AMPInter(
    ...
    def InterDst(self) -> nn.Module:
        """Create destination inter module."""
        return _AMPInter(
            in_features=self.flatten_size,
            ae_dims=self.ae_dims,
            ae_out_ch=self.ae_dims,
...
class _AMPEncoder(nn.Module):
    """AMP Encoder implementation."""
...
class _AMPInter(nn.Module):
    """AMP Inter (InterSrc or InterDst) implementation."""
...
class _AMPDecoder(nn.Module):
    """AMP Decoder implementation (shared decoder)."""
